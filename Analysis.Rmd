---
title: "Larval and Recruit Responses"
...
2. There is a signiicant interaction between day and treatment, specifically that there is a significant reduction in survival in cool temperature over time as compared to ambient over time and there is a reduction in high compared to ambient. 
...
LarvalSettlePlot<-ggplot(data=larval_settle_table, aes(x=Day, y=mean, colour=Treatment)) + 
  ...
  geom_text(x=4, y=75, label="p(Larval Treatment x Day)<0.001", size=6, color="black") +
...
Add individual aggregate section here when finalized: 
...
#Survival is significantly different b/t high and ambient at day 28 and day 47
...
## Account for the effect of colony type (individual vs aggregate) on survival    

The success and failure columns in the `recruits` data frame reflect the total survival (recruits + individuals). Generate a new success and failure column for individuals and aggregates separately.  
...
Run model on only indiviual colonies (removing aggregates).  
```{r}
#agg_ind$Days<-as.factor(as.character(agg_ind$Days))
attach(agg_ind)

y<-cbind(Success_Ind, Failure_Ind)

model5<-glmer(y~Juv.Treatment * Larval.Treatment * Days + (1|Tank/Plug.ID), subset=c(Failure_Ind>0), family=binomial, data=agg_ind) 
summary(model5) 
Anova(model5, type=2)
posthoc5<-emmeans(model5, ~Juv.Treatment|Days, adjust="Tukey")
cld(posthoc5)
pairs(posthoc5)

#Survival is significantly different b/t high and ambient at day 28 and day 47

dispersion_glmer(model5) #no evidence of overdispersion as value is <1.4
...
ggsave(filename="Figures/DataFigure.pdf", plot=figure2b, dpi=500, width=12, height=10, units="in")
ggsave(filename="Figures/DataFigure.png", plot=figure2b, dpi=500, width=12, height=10, units="in")
