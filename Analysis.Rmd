---
title: "Analysis of Larval and Recruit Responses"
...
Calculate percent change in larval concentration relative to starting concentration at each subsequent timepoint: (larvae end - larvae start) / larvae start  
...
Analyze larval density over time between treatments with a linear mixed model with concical nested within treatment as a random effect due to repeated measures.  
...
View summary and significance of model through a Type II ANOVA analysis.  
```{r}
...
Check residual asssumptions for the model.  
```{r}
...
Conduct posthoc examination of model.  
```{r}
...
LarvalSurvPlot<-ggplot(data=larvae_surv_table, aes(x=Day, y=mean, colour=Treatment)) + 
  ...
  geom_text(x=3.95, y=83, label="*", size=10, color="darkblue") + #day4 cool
  xlab(expression(bold("Days")));LarvalSurvPlot 
```

Overall, there are significant effects of the interaction between treatment and day:   
1. There is a significant reduction in survivorship over time (sign. effect of day). 
2. There is a signiicant interaction between day and treatment, specifically that there is a significant reduction in survival in cool temperature oat the ending timepoint as compared to ambient. There was also a trend for lower survival in high temperature, but this was not significant.   

# Larval settlement  

Load data for larval settlement. Calculate proportion of larvae settled in each chamber. Each chamber was loaded with a known number of larvae at the start of the settlement period. At each time point the number of recruits were counted and proportion settlement is calculated as the number of settled recruits at each time point divided by the number of starting larvae.     
```{r}
#load dataset
...
Analyze "total" settlement over time between treatments with a linear mixed model. As all chambers started with 100 larvae, we are not using the vector of success and failure. Because this is count data we will use a poisson distribution in a glmer model. Nest chamber within tank as this is repeated measures.  
...
View model summary and significance using a Type II ANOVA analysis.  
```{r}
summary(model2) 
Anova(model2, type=2)
```
...
Conduct posthoc analyses to look for pairwise differences in settlement between treatments at each day.  
```{r}
...
LarvalSettlePlot<-ggplot(data=larval_settle_table, aes(x=Day, y=mean, colour=Treatment)) + 
  ...
  geom_text(x=1.05, y=23, label="*", size=10, color="darkblue") + #day1 cool
  geom_text(x=2.05, y=22, label="**", size=10, color="red") + #day2 high
  geom_text(x=2.05, y=19, label="*", size=10, color="darkblue") + #day2 high
  geom_text(x=3.05, y=29, label="**", size=10, color="red") + #day3 high
  geom_text(x=3.05, y=25, label="**", size=10, color="darkblue") + #day3 cool
...
1. There is lower settlement in cool temp compared to ambient and higher settlement in high as compared to ambient (sign. effect of treatment).  
...
Load data for recruit survival. Calculate percent recruit survival as the number of recruits alive at each timepoint divided by the number of recruits alive at the start of the experiment. 

In this analysis, the number of recruits alive at each time point is recorded as the number of "successes". The number of larvae that died are "failures". Recruits are tracked by "plug ID".  
```{r}
...
Generate a summary table of recruit survival over time (days 0, 1, 3, 4) in ambient and high conditions originating from ambient, cool, and high larval treatments treatments. In this experiment recruits that originated from each larval treatment were exposed to ambient and high recruit treatments.    
```{r}
...
Analyze recruit survival over time between treatments with a binomial mixed model with a success and failure vector. Plug included as a random effect due to repeated measures. Proportion aggregation for each plug also included as a random effect to account for variation in settlement behavior.  

Build model for recruit survival: `glmer(y~Juv.Treatment * Larval.Treatment * Days + (1|Tank/Plug.ID), family=binomial, data=recruits)`
```{r}
...
View summary of model and significance with a Type II ANOVA analysis.  
```{r}
summary(model3) 
Anova(model3, type=2)
```

Run posthoc analysis to look at differences between juvenile treatments for each day because day*juvenile treatment was significant, with no effect of larval treatment.  
```{r}
...
We now need to account for the effect of colony type (individual vs aggregate) on survival. We found variation in settlement behavior with recruits settling both as individuals and aggregates of multiple larvae. The number of aggregates and individuals was recorded and the survival of each individual tracked.    

The success and failure columns reflect the total survival (recruits + individuals). Generate a new success and failure column for individuals and aggregates separately. Displaymean proportion aggregation for each treatment combination.  
```{r}
...
#calculate proportion of colonies that are aggregates for these plugs to look at how this might impact results 
agg_ind<-agg_ind%>%
...
Anova(model4, type=2)
```
Results show the same significant effects as the previous model - juvenile treatment and day interaction is significant.  

Examine posthoc differences between juvenile treatments at each day.  
```{r}
...
RecruitSurvPlot<-ggplot(data=recruit_surv_table, aes(x=Days, y=mean, shape=Juv.Treatment, linetype=Juv.Treatment, colour=Larval.Treatment, group=interaction(Juv.Treatment, Larval.Treatment))) + 
  ...
  scale_linetype_manual(name="Juvenile Treatment", values=c("solid", "dotted"), labels=c("Ambient", "High"))+
  scale_shape_manual(name="Juvenile Treatment", values=c(19,21))+ 
  theme_classic()+ 
  ylim(0,100)+
  xlim(0,50)+
  theme(text = element_text(size = 18, color="black"))+ 
  ...
  geom_text(x=25, y=25, label="p(Juvenile Treatment x Day)<0.001", size=6, color="black") +
...
figure_surv<-plot_grid(LarvalSurvPlot, LarvalSettlePlot, labels = c("a", "b"), ncol=2, nrow=1, rel_heights= c(1,1), rel_widths = c(1,1), label_size = 20, label_y=1, align="h");figure_surv

figure_surv_b<-plot_grid(figure_surv, RecruitSurvPlot, labels = c("", "c"), ncol=1, nrow=2, rel_heights= c(1,1), rel_widths = c(1,1), label_size = 20, label_y=1);figure_surv_b
