...
Load data for spat survival. Calculate percent spat survival as the number of spat alive at each timepoint divided by the number of spat alive at the start of the experiment. 
...
Run posthoc analysis to look at differences between spat treatments for each day because day*spat treatment was significant, with no effect of larval treatment.  
```{r}
...
Anova(model4, type=2)
```
Results show the same significant effects as the previous model - spat treatment and day interaction is significant.  

Examine posthoc differences between spat treatments at each day.  
```{r}
