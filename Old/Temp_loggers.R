...
#TANKS ONLY
Tank1<-read.csv("Temperature/Tank1_5Sept.csv",header=TRUE, sep=",", na.strings="NA")
...
Tank2<-read.csv("Temperature/Tank2_5Sept.csv",header=TRUE, sep=",", na.strings="NA")
...
Tank3<-read.csv("Temperature/Tank3_5Sept.csv",header=TRUE, sep=",", na.strings="NA")
...
Tank4<-read.csv("Temperature/Tank4_5Sept.csv",header=TRUE, sep=",", na.strings="NA")
#Intercept: -2.757311e-14     slope= 1.0000000, line equation y=1  x +-2.757311e-14
...
TANK_plot<-ggplot(data=TANK_meanHOUR, aes(x=DateHour, y=mean, group = variable, color=variable)) + 
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1)+
  geom_point(size=1)+
  scale_color_manual(name, "Tank", values=c("#E69F00", "#56B4E9","#0072B2", "#D55E00"))+
  geom_line(aes(color=variable, linetype=variable))+
  ...
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        plot.background =element_blank(), #Set the plot background
        legend.key = element_blank()) + #Set plot legend key
  theme(plot.title = element_text(size=20,face = "bold",hjust = 0.5));TANK_plot
